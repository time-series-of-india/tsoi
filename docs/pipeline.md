...
| The Walk through Midnight (interactive film) | Maddison Project Database via Our World in Data; 2023–26 tail chained from IMF WEO + UN WPP growth rates | committed CSV in `data/independence/` → `build-independence.mjs` → `independence/economy.json` (the one DB-free surface; see `data/independence/SOURCES.md`) |
